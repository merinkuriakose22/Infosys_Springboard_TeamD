...
with col2:
    st.markdown('<div class="metric-card">', unsafe_allow_html=True)
    st.metric("Positive",
              f"{(pos_count/total_reviews*100 if total_reviews > 0 else 0):.1f}%",
              delta=f"{pos_count}")
    st.markdown('</div>', unsafe_allow_html=True)
...
with col3:
    st.markdown('<div class="metric-card">', unsafe_allow_html=True)
    st.metric("Negative",
              f"{(neg_count/total_reviews*100 if total_reviews > 0 else 0):.1f}%",
              delta=f"{neg_count}",
              delta_color="inverse")
    st.markdown('</div>', unsafe_allow_html=True)
...
with col4:
    st.markdown('<div class="metric-card">', unsafe_allow_html=True)
    st.metric("Neutral",
              f"{(neu_count/total_reviews*100 if total_reviews > 0 else 0):.1f}%",
              delta=f"{neu_count}",
              delta_color="off")
    st.markdown('</div>', unsafe_allow_html=True)
...
with c_left:
    ...
    if not filtered_df.empty:
        ...
        counts = filtered_df["sentiment"].value_counts().reindex(
...
with c_right:
    ...
    if not filtered_df.empty:
        ...
        ax_hist.hist(filtered_df["confidence_score"],
                     bins=20,
...
if not filtered_df.empty:
    ...
    product_sent = filtered_df.groupby('product')['sentiment']\
    ...
    sns.heatmap(
        product_sent[['Positive','Neutral','Negative']],
        annot=True,
        fmt="d",
        cmap="RdYlGn",
        ax=ax_hm
...
if not keywords_df.empty:
    ...
    with col_bar:
        ...
        ax_bar.barh(
            keywords_df['keyword'].head(15),
            keywords_df['frequency'].head(15),
            color='skyblue'
        )
        ax_bar.invert_yaxis()
        st.pyplot(fig_bar)
...
st.success("✅ Dashboard ready! Use the sidebar filters to explore insights.")
